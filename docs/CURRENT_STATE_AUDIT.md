...
### System Log (`navGroup: 'log'`) — 7 views, all read-only

| id | title | aspPage | Description | Reads | Write | Confidence |
|---|---|---|---|---|---|---|
| `log-general` | General Log | Main_LogStatus_Content.asp | Full syslog text with client-side line filter, download-as-.txt, and 5s-optional auto-refresh. | syslog feed (`fetchSyslog`) | none — read-only display | read: live-verified |
| `log-wireless` | Wireless Log | Main_WStatus_Content.asp | Wireless-status hook output rendered as parsed tables, falling back to a raw-text dump if the model-specific hook output isn't array-shaped. 8s-optional auto-refresh. | `fetchWirelessStatus` (hook-derived arrays or raw text) | none — read-only display | read: structural |
| `log-dhcp` | DHCP Leases | Main_DHCPStatus_Content.asp | Active DHCP leases: hostname, MAC, IP, remaining lease time. 10s-optional auto-refresh. | DHCP leases (`fetchDhcpLeases`) | none — read-only display | read: live-verified |
| `log-ipv6` | IPv6 | Main_IPV6Status_Content.asp | IPv6 configuration summary + IPv6 LAN client table. Gated on `rcSupport` having `ipv6`. | `fetchIpv6Status` | none — read-only display | read: structural |
| `log-routes` | Routing Table | Main_RouteStatus_Content.asp | IPv4 and IPv6 kernel routing tables (destination/gateway/flags/metric/interface). | `fetchRoutes` | none — read-only display | read: structural |
